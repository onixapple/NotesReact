...
import { Badge, Button, Card, CardBody, Col, Form, Modal, Row, Stack } from "react-bootstrap";
...
export function NoteList({ availableTags, notes, onDeleteTag, onUpdateTag }: NoteListProps) {
    ...
    return <>
        ...
        <EditTagsModal onDeleteTag={onDeleteTag} onUpdateTag={onUpdateTag} availableTags={availableTags} show= {editTagsModalIsOpen} handleClose={() => setEditTagsModalIsOpen(false)}/>
    </>
}


function NoteCard({ id, title, tags }: SimplifiedNote) {
    return <Card as={Link} to={`${id}`} className={`h-100 text-reset text-decorations-none ${styles.card}`}>
        <Card.Body>
            <Stack className="align-items-center justify-content-center h-100" gap={2}>
